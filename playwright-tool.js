 ...
 *
 * Each call to createPlaywrightServer() returns an isolated instance with
 * its own browser/context/page — no module-level globals, so concurrent
 * QwenBridge instances never collide.
 */
const { tool, createSdkMcpServer } = require('@qwen-code/sdk')
const { z } = require('zod')

// ── factory: one isolated browser per server instance ─────────────────────────

function createPlaywrightServer() {
  // Instance-scoped state — each server gets its own browser lifecycle
  let _browser = null
  let _context = null
  let _page = null
  ...
  /** Expose the raw browser ref so QwenBridge can track it for cleanup. */
  function getBrowser() { return _browser }

  // ── tool definitions (close over instance state) ────────────────────────────
  ...
  // ── build and return the MCP server + cleanup handle ────────────────────────

  const server = createSdkMcpServer({
    name: 'playwright',
    version: '1.0.0',
    tools: [
      navigateTool, screenshotTool, clickTool, typeTool,
      getTextTool, getHtmlTool, evaluateTool, waitForTool,
      selectOptionTool, closeBrowserTool,
    ],
  })

  // Attach helpers so the caller (QwenBridge) can manage the lifecycle
  server._closeBrowser = closeBrowser
  server._getBrowser = getBrowser
