...
- [x] 5. Modify `orchestrator.js` — Safe-edit workflow injection
  - [x] 5.1 Add `SAFE_EDIT_INSTRUCTIONS` constant and inject into implementation agent prompts
  ...
  - [x] 5.2 Wire `lspManager` into Orchestrator from `main.js`
  ...
  - [x] 5.3 Write unit tests for orchestrator safe-edit injection (`test/orchestrator.test.js`)
...
- [x] 6. Add symbol browser UI to renderer
  - [x] 6.1 Add "Symbols" sub-section to the file explorer sidebar in `renderer/app.js`
  ...
  - [x] 6.2 Add symbol popover to LSP status chip click
  ...
  - [x] 6.3 Add CSS styles for symbol browser and status popover in `renderer/style.css`
...
- [x] 7. Align `agent-pool.js` with LSP tool sets
  - [x] 7.1 Add dynamic LSP tool merging in `registerType` or `selectType`
  ...
  - [x] 7.2 Update `main.js` agent pool registration to wire LSP status
  ...
  - [x] 7.3 Write unit tests for agent pool LSP tool merging (`test/agent-pool.test.js`)
