...
- [ ] 1. Create `lsp-manager.js` — LSP process lifecycle manager
  - [ ] 1.1 Implement `LspManager` class with constructor, status enum, and EventEmitter base
  ...
  - [ ] 1.2 Implement binary discovery and language server detection
  ...
  - [ ] 1.3 Implement `start(projectDir)` — spawn agent-lsp via `child_process.spawn` with stdio
  ...
  - [ ] 1.4 Implement `call(toolName, args)` — JSON-RPC request/response over stdio
  ...
  - [ ] 1.5 Implement health check, restart policy, and `stop()`
  ...
  - [ ] 1.6 Write unit tests for `LspManager` (`test/lsp-manager.test.js`)
  ...
  - [ ] 1.7 Write property tests for `LspManager` (`test/lsp-manager.property.test.js`)
...
- [ ] 2. Checkpoint — LspManager core
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 3. Create `main/ipc-lsp.js` — IPC handlers for LSP operations
  - [ ] 3.1 Implement IPC handler module with `register(ipcMain, ctx)` pattern
  ...
  - [ ] 3.2 Add LSP IPC channels to `preload.js`
...
- [ ] 4. Modify `direct-bridge.js` — LSP tool routing and edit hooks
  - [ ] 4.1 Add `lsp_` tool routing branch in `executeTool`
  ...
  - [ ] 4.2 Add speculative edit hook before `write_file` execution
  ...
  - [ ] 4.3 Add post-edit diagnostic hook after `write_file`/`edit_file`
  ...
  - [ ] 4.4 Add tool list filtering based on LSP status and agent role
  ...
  - [ ] 4.5 Enhance `buildProjectContext` with LSP symbol outlines
