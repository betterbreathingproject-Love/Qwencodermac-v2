...
- [x] 1.1 Update `get_engine()` to initialize `FusionEngine(rewind=True)` and cache the instance
- [x] 1.2 Update `cmd_compress_messages()` to forward per-message `contentType` hints and `dedup` option to `FusionEngine.compress_messages()`
- [x] 1.3 Update `cmd_compress_messages()` to include per-message stats (`original_tokens`, `compressed_tokens`, `reduction_pct`, `stages_applied`) in JSON output
- [x] 1.4 Update `cmd_compress_text()` to return `rewind_key` and `stages_applied` in stats when RewindStore is enabled
- [x] 1.5 Add `cmd_rewind()` function that calls `FusionEngine.rewind(key)` and returns original content or error
- [x] 1.6 Add `rewind` command to the `__main__` dispatch block
- [x] 1.7 Update `cmd_status()` to include `rewind_enabled` in output
- [x] 1.8 Wrap all FusionEngine calls in try/except to return original content + error field on failure

## Task 2: Update Node.js Compactor Module for v7 Features
- [x] 2.1 Update `compressMessages()` to forward `contentType` per message and `dedup` option in the stdin JSON payload
- [x] 2.2 Update `compressText()` to return `rewind_key` from the Python bridge response
- [x] 2.3 Add `rewind(pythonPath, key)` function that calls the Python bridge `rewind` command
- [x] 2.4 Update `compressMessages()` and `compressText()` to include `engine: 'python'` in stats on success, `engine: 'builtin'` on fallback
- [x] 2.5 Update `getStatus()` to include `rewind_enabled` field

## Task 3: Add `detectContentType` Helper to `direct-bridge.js`
- [x] 3.1 Implement `detectContentType(toolName, content)` function with tool-name mapping (`read_file` → `code`, `search_files`/`grep_search` → `search`, `bash`/`execute_command`/`list_dir` → `log`, browser tools → `prose`)
- [x] 3.2 Add JSON content override: if content starts with `{` or `[` and parses as JSON, return `'json'`
- [x] 3.3 Add diff content override: if content contains `---`, `+++`, `@@` markers, return `'diff'`
