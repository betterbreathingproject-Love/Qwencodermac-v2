...
- [ ] 1. Create memory-bridge.py with initialization, status, and secret filtering
  - [ ] 1.1 Create `memory-bridge.py` as a FastAPI APIRouter module with `initialize()` and `shutdown()` lifecycle functions
  ...
  - [ ] 1.2 Implement `GET /memory/status` and `GET /memory/stats` endpoints
  ...
  - [ ] 1.3 Implement the secret filtering pipeline as a preprocessing function
  ...
  - [ ] 1.4 Register the memory router in `server.py` via `app.include_router()` and wire SIGTERM handler
  ...
  - [ ]* 1.5 Write property test for secret filtering (Property 16)
...
- [ ] 2. Implement Knowledge Graph API endpoints
  ...
  - [ ] 2.2 Implement `GET /memory/kg/query/{entity}` and `POST /memory/kg/query-temporal` endpoints
  ...
  - [ ] 2.3 Implement `DELETE /memory/kg/clear` endpoint with confirmation guard
    - Require `confirm: true` in request body; reject without it
    - _Requirements: 14.1_

  - [ ]* 2.4 Write property tests for Knowledge Graph (Properties 1, 2, 3, 17)
...
- [ ] 3. Implement Archive API endpoints
  - [ ] 3.1 Implement `POST /memory/archive/record` endpoint
  ...
  - [ ] 3.2 Implement `GET /memory/archive/search` and `GET /memory/archive/events` endpoints
  ...
  - [ ] 3.3 Implement `POST /memory/archive/compress` endpoint
    - Trigger gzip compression of archive JSONL files older than 24 hours
    - _Requirements: 14.3_

  - [ ]* 3.4 Write property tests for Archive (Properties 4, 5, 6)
...
- [ ] 4. Implement Vector Memory and Unified Retrieval endpoints
  - [ ] 4.1 Implement `POST /memory/vector/add` and `POST /memory/vector/search` endpoints
  ...
  - [ ]* 4.3 Write property tests for Vector Memory and Retrieval (Properties 7, 8, 9)
...
- [ ] 6. Implement dual-model MLX serving and fact extraction endpoints
  - [ ] 6.1 Implement `POST /memory/extractor/load` and `POST /memory/extractor/unload` endpoints
  ...
  - [ ] 6.2 Implement `POST /memory/extract` and `GET /memory/extract/queue` endpoints
  ...
  - [ ]* 6.3 Write property test for extraction pipeline (Property 10)
    - **Property 10: Extraction pipeline stores in KG and vector**
    - File: `test/memory-bridge.property.test.js`
    - **Validates: Requirements 7.4, 7.5**

- [ ] 7. Implement session management endpoints
  - [ ] 7.1 Implement `POST /memory/session/enrich` and `POST /memory/session/crystallize` endpoints
  ...
  - [ ]* 7.2 Write unit tests for session management endpoints
    - Test enrichment with and without Extraction_Model
    - Test crystal generation and KG feedback
    - File: `test/memory-bridge.test.js`
    - _Requirements: 13.3, 13.4, 13.5_

- [ ] 8. Create memory-client.js Node.js client module
  - [ ] 8.1 Create `memory-client.js` with HTTP helper and all exported async functions
  ...
  - [ ]* 8.2 Write property test for memory client error resilience (Property 11)
  ...
  - [ ]* 8.3 Write unit tests for memory-client.js
...
- [ ] 9. Checkpoint — Ensure all client and backend tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 10. Integrate memory retrieval and extraction into the agent loop (direct-bridge.js)
  - [ ] 10.1 Add recall phrase detection and pre-LLM memory retrieval to `_agentLoop()`
  ...
  - [ ]* 10.2 Write property tests for recall phrase detection and context injection (Properties 12, 13, 14)
  ...
  - [ ] 10.3 Add post-turn async fact extraction to `_agentLoop()`
  ...
  - [ ] 10.4 Add tool call and decision archiving to `_agentLoop()`
  ...
  - [ ]* 10.5 Write property tests for tool call archiving (Properties 18, 19)
    - **Property 18: Tool call archive truncation**
    - **Property 19: Tool call archive metadata completeness**
    - File: `test/memory-client.property.test.js`
    - **Validates: Requirements 12.2, 12.5**

  - [ ] 10.6 Add session start/end lifecycle events
...
- [ ] 11. Integrate archive-before-compact into the agent loop
  - [ ] 11.1 Add archive-before-compact logic to `_agentLoop()` compaction trigger
  ...
  - [ ]* 11.2 Write integration tests for archive-before-compact flow
...
- [ ] 12. Integrate memory into the orchestrator
  - [ ] 12.1 Add pre-dispatch memory retrieval and post-completion archiving to `orchestrator.js`
  ...
  - [ ]* 12.2 Write property test for orchestrator specContext augmentation (Property 15)
    - **Property 15: Orchestrator specContext augmentation**
    - File: `test/orchestrator-memory.test.js`
    - **Validates: Requirements 10.2**

  - [ ]* 12.3 Write integration tests for orchestrator memory integration
